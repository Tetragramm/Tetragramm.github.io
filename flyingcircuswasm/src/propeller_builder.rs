...
impl PropellerBuilder {
    ...
    fn calc_power(&self) -> f32 {
        ...
        rtz((1.0e-6 + output_force / era.power_div) as f64) as f32
    }
    ...
    fn calc_drag(&self) -> f32 {
        ...
        rtz((1.0e-6 + raw_drag * self.cool_drag() * self.upgrade_drag()) as f64) as f32
    }
    ...
    fn calc_cooling(&self) -> f32 {
        ...
        let cool = &COOLING_TABLE[self.cool_sel];
        rtz((1.0e-6 + self.material_modifier() / 20.0 * cool.radiator) as f64) as f32
    }
    ...
    fn calc_fuel_consumption(&self) -> f32 {
        ...
        rtz((1.0e-6 + fuel_consumption * self.upgrade_fuel()) as f64) as f32
    }
    ...
    fn cool_torque(&self) -> f32 {
        ...
        if cool.name == "Rotary" {
            ...
            rtz((1.0e-6 + self.calc_mass() / 2.0) as f64) as f32
        } else {
            1.0
        }
    }
    ...
    fn calc_torque(&self) -> i16 {
        rtz((1.0e-6 + (self.cool_torque() * self.geared_rpm() / 5.0) / 4.0) as f64) as i16
    }
    ...
    fn calc_cost(&self) -> f32 {
        ...
        rtz((1.0e-6 + plus_bs_and_era) as f64) as f32
    }
    ...
    pub fn build(&mut self) -> EngineStats {
        ...
        estats.stats.fuelconsumption = self.calc_fuel_consumption() as f64;
        estats.altitude = self.calc_altitude();
        estats.torque = self.calc_torque();
        estats.stats.cost = self.calc_cost() as f64;
        ...
        match self.compressor_type {
            ...
            CompressorType::Supercharger => {
                ...
                estats.stats.reliability -= extra as f64;
                estats.stats.mass += extra as f64;
                estats.stats.drag += extra as f64;
                estats.stats.cost += (2 * extra) as f64;
            }
            CompressorType::Turbocharger => {
                estats.stats.power = rtz(1.0e-6 + 1.25 * estats.stats.power) as f64;
                estats.stats.mass = rtz(1.0e-6 + 1.2 * estats.stats.mass) as f64;
                estats.stats.drag += (2 * (self.min_iaf / 10)) as f64;
                estats.stats.cost += 1.0 + rtz(1.0e-6 + estats.stats.power / 50.0) as f64;
...
mod tests {
    ...
    #[test]
    fn test_himmilgard_liquid_radial() {
        ...
        let test_cases = vec![
            ...
            (
                ...
                (14.0, 7.0, 9.0, 3.0, -1.0, 20.0, 26, 11.0),
            ),
            // Einsort MR 150hp
            (
                r#"{
                    "name": "Einsort MR 150hp",
                    "engine_type": 0,
                    "type": 5,
                    "era_sel": 1,
                    "displacement": 15.46,
                ...
                (15.0, 7.0, 9.0, 3.0, -1.0, 21.0, 26, 11.0),
            ),
            // Gernsback Motor 0005
            (
                r#"{
                    "name": "Gernsback Motor 0005",
                    "engine_type": 0,
                    "type": 5,
                    "era_sel": 1,
                    "displacement": 12,
                ...
                (21.0, 8.0, 6.0, 5.0, -1.0, 20.0, 32, 10.0),
            ),
        ];
        ...
        ) in test_cases
        {
            ...
            let mut builder = PropellerBuilder::from_inputs(&inputs);
            let stats = builder.build();

            // Extract engine name for error messages
            let engine_name = &stats.name;

            // Compare stats (allow small floating point differences)
            assert_eq!(
