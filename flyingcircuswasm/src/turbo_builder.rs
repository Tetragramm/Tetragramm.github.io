...
impl TurboBuilder {
    ...
    pub fn from_inputs(inputs: &EngineInputs) -> TurboBuilder {
        ...
        } = &inputs.inputs
        {
            ...
            builder.flow_adjustment = *flow_adjustment as f64;
            builder.diameter = *diameter as f64;
            builder.compression_ratio = *compression_ratio as f64;
            builder.bypass_ratio = *bypass_ratio as f64;
    ...
    fn calc_drag(&self) -> f64 {
        rtz(1.0e-6 + 5.0 * std::f64::consts::PI * (self.diameter / 2.0).powi(2))
    }
    ...
    fn mfp(&self, m: f64) -> f64 {
        const R: f64 = 287.058 / 1000.0; // Gas Constant
        const Y: f64 = 1.4; // Specific Heat

        (Y / R).sqrt() * m * (1.0 + (Y - 1.0) / 2.0 * m * m).powf((Y + 1.0) / (2.0 * (Y - 1.0)))
    }

    /// Calculate stages (thrust and fuel consumption)
    fn calc_stages(&self) -> (f64, f64) {
        let era = &ERA_TABLE[self.era_sel];

        const M: f64 = 0.0;
        const A0: f64 = 340.3;
        const PA: f64 = 108.9; // Ambient Pressure
        const TA: f64 = 288.15; // Ambient Temp
        const CP: f64 = 1.006; // Specific Heat at Constant Pressure
        const Y: f64 = 1.4; // Specific Heat
