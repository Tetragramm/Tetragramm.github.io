...
impl Rotor {
    ...
    pub fn get_power_factor(&self) -> f64 {
    ...
    pub fn get_rotor_area(&self) -> f64 {
        let total_span = (self.sizing_span + self.rotor_span) as f64;
        (PI / 9.0) * total_span * total_span
    }

    /// Calculate ideal rotor area (without additional span)
    /// TypeScript: GetIdealRotorArea()
    pub fn get_ideal_rotor_area(&self) -> f64 {
        let span = self.sizing_span as f64;
        (PI / 9.0) * span * span
    }
    ...
    pub fn get_rotor_drag(&self) -> i16 {
            ...
            || self.aircraft_type == AircraftType::Autogyro
        {
            ...
            if self.rotor_count == 1 {
                ...
                (0.75 * self.rotor_count as f64 * single_drag).floor() as i16
            }
        } else {
            0
        }
    ...
    pub(super) fn get_rotor_strain(&self) -> f64 {
        let area = self.get_rotor_area().floor();
        let span_sum = (self.sizing_span + self.rotor_span) as f64;
        let base_strain = (2.0 * span_sum + area - 10.0).max(1.0);
        self.rotor_count as f64 * base_strain
    }
}
