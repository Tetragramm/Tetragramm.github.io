...
impl WeaponSystem {
    ...
    pub fn get_wing_weight(&mut self) -> f64 {
        self.weapons
            .iter_mut()
            .filter(|w| w.get_wing())
    ...
    pub fn get_hits(&self) -> [i16; 4] {
        ...
        if hits != 0 {
            ...
            [
                centerline + wings,
                ((1.0e-6 + centerline as f64 * 0.75).floor() as i16)
                    + ((1.0e-6 + wings as f64 * 0.9).floor() as i16),
                ((1.0e-6 + centerline as f64 * 0.5).floor() as i16)
                    + ((1.0e-6 + wings as f64 * 0.2).floor() as i16),
                ((1.0e-6 + centerline as f64 * 0.25).floor() as i16)
                    + ((1.0e-6 + wings as f64 * 0.1).floor() as i16),
            ]
        } else {
            if self.is_lightning_arc() {
                [0, 0, 0, 0]
    ...
    pub fn get_hr_charges(&self) -> Vec<i16> {
        ...
        if self.final_weapon.hits > 0 {
            ...
            if self.final_weapon.rapid {
                vec![
                    count * ammo,
                    (count * ammo + 1).max((1.5 * count as f64 * ammo as f64) as i16),
                ]
            } else {
                vec![count * ammo]
            }
