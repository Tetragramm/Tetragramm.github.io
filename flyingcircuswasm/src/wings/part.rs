...
impl Part for Wings {
    ...
    fn part_stats(&mut self) -> Stats {
        ...
        for wing in &self.wing_list {
            ...
            wing_stats = wing_stats.add(
                &self.skin_list[wing.surface]
                    .stats
                    .multiply(wing.area as f64),
            );
            wing_stats.wingarea = wing.area as f64;

            // Wing structure stress
            wing_stats.maxstrain += 0.0_f64.min(-((2 * wing.span + wing.area - 10) as f64));
            ...
            wing_stats.latstab += (wing.dihedral - wing.anhedral) as f64;
            wing_stats.liftbleed += (wing.dihedral + wing.anhedral) as f64;
        ...
        stats.control += (8 - longest_span) as f64;
        stats.latstab += 0.0_f64.min((longest_span - 8) as f64);
        ...
        if self.stagger_list[self.wing_stagger].inline {
            for count in &deck_count {
                if *count > 1 {
                    stats.liftbleed += ((*count - 1) * 3) as f64;
                }
            }
        }
        ...
        if self.is_closed {
            let pairs = (self.wing_list.len() as f64 / 2.0).floor() as i16;
            stats.mass += pairs as f64;
            stats.control -= (5 * pairs) as f64;
            stats.maxstrain += (20 * pairs) as f64;
        }
    ...
    fn get_electrics(&self) -> ElectricsMessage {
        ...
        for wing in &self.wing_list {
            ...
            if skin.stats.charge != 0.0 {
                ...
                total_charge += skin.stats.charge * wing.area as f64;
