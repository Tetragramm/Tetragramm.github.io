...
import { BlinkBad, BlinkNeutral, createMobileOptionItem } from "../../../Test/src/wasm/dom_utils"
import { JSON2CSV } from "../../../Test/src/JSON2CSV/json2csv";
    ...
    hangar_load.onchange = async (evt) => {
        ...
        reader.onloadend = async () => {
            try {
                ...
                AddHangar(name);
                window.localStorage.setItem("helicopter.hangar." + name, reader.result as string);
                chosen_hangar = name;
                RefreshHangarSelect(LoadHangarList());
                RefreshAcftSelect(LoadAcftList());
                await LoadFromHangar(select_acft.selectedIndex);
...
async function InitStats() {
    const acft_data = window.localStorage.getItem("helicopter.aircraft");
...
function LoadHangarList(): string[] {
    if (!window.localStorage.getItem("helicopter.hangar_names")) {
        window.localStorage.setItem("helicopter.hangar_names", JSON.stringify(["Default"]));
    }
    let hangar_list: string[] = JSON.parse(window.localStorage.getItem("helicopter.hangar_names"));
    if (hangar_list.length == 0) {
        window.localStorage.setItem("helicopter.hangar_names", JSON.stringify(["Default"]));
        hangar_list = JSON.parse(window.localStorage.getItem("helicopter.hangar_names"));
    }
    return hangar_list;
}
...
function LoadAcftList(): { names: string[], acft: string[] } {
    ...
    if (window.localStorage.getItem("helicopter.hangar." + chosen_hangar)) {
        acft_list = JSON.parse(window.localStorage.getItem("helicopter.hangar." + chosen_hangar));
    } else {
        acft_list = {
            names: ["Basic Biplane"],
        ...
        window.localStorage.setItem("helicopter.hangar." + chosen_hangar, JSON.stringify(acft_list));
    }
    return acft_list;
}
...
function RemoveHangar(name: string) {
    ...
    if (idx != -1) {
        ...
        window.localStorage.removeItem("helicopter.hangar." + name);
    }
    SaveHangarList(hangar_list);
    chosen_hangar = "Default";
    RefreshHangarSelect(LoadHangarList());
...
function SaveHangarList(hangar_list: string[]) {
    window.localStorage.setItem("helicopter.hangar_names", JSON.stringify(hangar_list));
