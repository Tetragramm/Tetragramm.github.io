    ...
    fn section_stats(&self, sec: &Section) -> Stats {
        ...
        if !sec.internal_bracing {
            ...
            if sec.monocoque {
                stats.mass = 0.0;
                stats.cost += 1.0;
                stats.structure = self.skin_list[self.sel_skin].monocoque_structure as f32;
            }
...
impl Part for Frames {
    fn part_stats(&mut self) -> Stats {
        ...
        stats.structure = self.frame_list[base_type].basestruct as f32;
        stats.cost = self.frame_list[base_type].basecost as f32;
        ...
        if self.farman {
            ...
            stats.visibility += self.tail_section_list.len() as f32;
        ...
        stats.cost += lb_count as f32;

        if self.flying_wing {
            stats.liftbleed += 5.0;
        } else {
            stats.drag += lb_count as f32;
        }
