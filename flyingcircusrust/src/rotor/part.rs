...
impl Part for Rotor {
    ...
    fn part_stats(&mut self) -> Stats {
        ...
        // Add rotor drag
        stats.drag = self.get_rotor_drag() as f32;
        ...
        if self.aircraft_type == AircraftType::Helicopter {
            ...
            stats.reliability = 2.0 * (0.0_f32.min(self.rotor_span as f32));

            // Add blade count stats
            stats = stats.add(&self.blade_list[self.blade_idx].stats);

            // Rotor thickness effects
            stats.power -= self.rotor_thickness as f32;
            stats.maxstrain += 10.0 * self.rotor_thickness as f32;
        }
        ...
        if self.accessory {
            if self.aircraft_type == AircraftType::Autogyro {
                ...
                stats.mass += (self.rotor_count * self.engine_count) as f32;
            }
        } else if self.aircraft_type == AircraftType::Helicopter {
            stats.mass += self.rotor_count.max(self.engine_count) as f32;
        }
