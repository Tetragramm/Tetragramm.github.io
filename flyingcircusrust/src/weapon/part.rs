...
impl Part for Weapon {
    ...
    fn part_stats(&mut self) -> Stats {
        ...
        // Covered cost
        if self.covered {
            let mut cost: f32 = if self.weapon_type.size <= 1 {
                0.0
        ...
        if self.wing && !self.covered {
            stats.drag += self.w_count as f32;
        }
        ...
        // Accessible cost
        if self.accessible {
            stats.cost += ((self.w_count as f32 / 2.0).floor()).max(1.0);
        }
        ...
        match self.synchronization {
            SynchronizationType::Interrupt => {
                stats.cost += (self.w_count * 2) as f32;
            ...
            SynchronizationType::Synch => {
                if self.action != ActionType::Mechanical {
                    stats.cost += (self.w_count * 3) as f32;
