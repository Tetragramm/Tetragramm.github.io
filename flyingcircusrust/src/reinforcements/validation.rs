...
impl Reinforcements {
    ...
    pub fn verify_cant_limits(&mut self) {
        for i in 0..self.cant_list.len() {
            if self.cant_list[i].limited && self.cant_count[i] > 0 {
                ...
                if cant_mass > 0.0 {
                    let max_allowed = (available_structure as f32 / (5.0 * cant_mass)) as i16;
    ...
    pub(super) fn available_structure(&self) -> i16 {
        let mut total: f32 = self.acft_structure as f32;
        total += self.cabane_list[self.cabane_sel as usize].stats.structure;

        // Add external wood strut structure
        for i in 0..self.ext_wood_count.len() {
            total += self.ext_wood_count[i] as f32 * self.ext_wood_list[i].stats.structure;
            if i < self.ext_steel_count.len() {
                total += self.ext_steel_count[i] as f32 * self.ext_steel_list[i].stats.structure;
            }
        }
        ...
        for i in 0..self.cant_list.len() {
            if self.cant_list[i].limited {
                total -= 5.0 * self.cant_count[i] as f32 * self.cant_list[i].stats.mass;
            }
        }
