...
impl Part for Reinforcements {
    fn part_stats(&mut self) -> Stats {
            ...
            if self.ext_wood_count[i] > 0 {
                ...
                stats = stats.add(
                    &self.ext_wood_list[i]
                        .stats
                        .multiply(self.ext_wood_count[i] as f32),
                );

                // Apply tension multiple for config struts
                let t = if self.ext_wood_list[i].config {
                    tension_multiple * self.ext_wood_list[i].tension as f32
                } else {
                    self.ext_wood_list[i].tension as f32
                };
                tension += t * self.ext_wood_count[i] as f32;
            }
        }
            ...
            if self.ext_steel_count[i] > 0 {
                ...
                stats = stats.add(
                    &self.ext_steel_list[i]
                        .stats
                        .multiply(self.ext_steel_count[i] as f32),
                );

                // Apply tension multiple for config struts
                let t = if self.ext_steel_list[i].config {
                    tension_multiple * self.ext_steel_list[i].tension as f32
                } else {
                    self.ext_steel_list[i].tension as f32
                };
                tension += t * self.ext_steel_count[i] as f32;
            }
        }
        ...
        if cabane_idx < self.cabane_list.len() {
            ...
            tension += tension_multiple * self.cabane_list[cabane_idx].tension as f32;
        ...
        for i in 0..self.cant_list.len() {
            if self.cant_count[i] > 0 {
                use_cant = true;
                stats = stats.add(&self.cant_list[i].stats.multiply(self.cant_count[i] as f32));
            }
        }
        ...
        if use_cant {
            stats.cost += 5.0;
            stats.liftbleed -= self.cant_lift as f32;
        }
