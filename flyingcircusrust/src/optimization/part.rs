...
impl Part for Optimization {
    ...
    fn part_stats(&mut self) -> Stats {
        ...
        stats.cost = (-(self.cost as f32 * self.acft_stats.cost / 10.0) + 1e-6).floor();

        // Bleed optimization: reduces liftbleed by 3 per dot
        stats.liftbleed = (-(self.bleed as f32) * 3.0 + 1e-6).floor();

        // Escape optimization: improves escape and visibility
        stats.escape = self.escape as f32;
        stats.visibility = self.escape as f32;

        // Mass optimization: reduces mass by 10% of aircraft mass per dot
        stats.mass = (-(self.mass as f32 * self.acft_stats.mass / 10.0) + 1e-6).floor();

        // Toughness optimization: increases toughness by 25% of aircraft toughness per dot
        stats.toughness =
            ((self.toughness as f32 * self.acft_stats.toughness / 4.0) + 1e-6).floor();

        // Maxstrain optimization gets applied later in derived stats
        // stats.maxstrain = ((self.maxstrain as f32 * 1.5 * self.acft_stats.maxstrain / 10.0) + 1e-6).floor();

        // Reliability optimization: +2 per dot
        stats.reliability = self.reliability as f32 * 2.0;

        // Drag optimization: reduces drag by 10% of aircraft drag per dot
        stats.drag = (-(self.drag as f32 * self.acft_stats.drag / 10.0) + 1e-6).floor();
