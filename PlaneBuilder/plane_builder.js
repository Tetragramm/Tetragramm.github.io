...
class Integer {
     ...
     */
    random_sample() {
        this.low = Math.trunc(this.low);
        this.high = Math.trunc(this.high);
        /* Returns a uniformly sampled value from the space */
 ...
 * objects.
 */
function to_space(space_object) {
...
function rs_minimize(func, dimensions, n_calls = 64, n_random_starts = 13, mutation_rate = 0.1) {
    var opt = new OMGOptimizer(dimensions, n_random_starts, mutation_rate);
    for (var iter = 0; iter < n_calls; iter++) {
        var x = opt.ask();
        var y = func(x);
        opt.tell([x], [y]);
...
*/
function minimize_Powell(fnc, x0) {
    var eps = 1e-2;
    ...
    var solution = { argument: x, fncvalue: fx };
    return solution;
}
;
/**
 ...
 * value of the function at the best found argument.
*/
function minimize_GradientDescent(fnc, grd, x0) {
 ...
 * value of the function at the best found argument.
*/
function minimize_L_BFGS(fnc, grd, x0) {
    // fnc: function which takes array of size N as an input
    // grd: gradient (array of size N) of function for some input
    // x0: array or real numbers of size N; 
    // serves as initialization of algorithm.
