...
import { HELICOPTER_DIR, HELICOPTER_HOST_DIR, isHelicopterPage, pageDir, storageKey } from './wasm/deployment';
...
export interface WasmApplicationConfig {
    /** localStorage key for saving/loading the active aircraft. Default: storageKey('aircraft'). */
    storageKey?: string;
     ...
     * the builder folder — the Rules pages and the Engine Builder. Default '' (the
     * builder's own pages, where the assets are siblings); the Helicopter pages
     * set '../Test/' (see HELICOPTER_HOST_DIR in wasm/deployment.ts).
     */
...
export class WasmApplication {
    ...
    private redirectIfWrongPage(jsonParam: string | null): boolean {
        if (!this.bridge) return false;
        const onHelicopterPage = isHelicopterPage();
        const isHelicopter = this.bridge.getAircraftType() === AircraftType.Helicopter;
        if (isHelicopter === onHelicopterPage) return false;

        // The Helicopter pages hand back to the builder copy they are bound to;
        // a builder copy (PlaneBuilder/ or Test/) hands off to the Helicopter page.
        const [fromDir, toDir] = onHelicopterPage
            ? [HELICOPTER_DIR, HELICOPTER_HOST_DIR]
            : [pageDir(), HELICOPTER_DIR];
        let targetUrl = window.location.href
            .replace(`/${fromDir}/index.html`, `/${toDir}/index.html`)
