...
import { storageKey } from './deployment';
...
export class AircraftBridge {
    ...
    private storageKey: string = storageKey('aircraft');
    ...
    loadEngineListsFromLocalStorage(): void {
        ...
        try {
            ...
            const engineNamesJson = localStorage.getItem(storageKey('engines_names'));
            if (!engineNamesJson) {
            ...
            for (const listName of engineListNames) {
                const enginesJson = localStorage.getItem(storageKey(`engines.${listName}`));
                if (enginesJson) {
                    try {
        ...
        try {
            ...
            for (const listName of allListNames) {
                ...
                if (engines && engines.length > 0) {
                    ...
                    localStorage.setItem(storageKey(`engines.${listName}`), JSON.stringify(list));
                    console.log(`[AircraftBridge] Saved ${engines.length} engines to list "${listName}"`);
                }
            }

            // Save the list of custom list names
            localStorage.setItem(storageKey('engines_names'), JSON.stringify(customListNames));
