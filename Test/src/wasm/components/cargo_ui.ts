 ...
 * Cargo UI Component
 *
 * Displays the Cargo section using UIBindings from Rust
 */
...
export class CargoUI {
    ...
    constructor(
        ...
        if (!bridge) {
            throw new Error('Bridge not available during CargoUI construction');
        }
    ...
    /**
     * Render the Cargo UI - intelligently updates or rebuilds
     */
    render(): void {
        const bridge = this.getBridge();
        if (!bridge || !bridge.isInitialized()) {
            console.warn('[CargoUI] Bridge not initialized, skipping render');
            return;
        }
    ...
    private rebuildFull(): void {
        ...
        const bridge = this.getBridge();
        if (!bridge || !bridge.isInitialized()) {
            console.warn('[CargoUI] Bridge not initialized, skipping rebuild');
            return;
        }

        const bindings = bridge.getCargoBindings();
        ...
        rulesLink.href = './Rules/Rules.htm#_Cargo';
        ...
        console.log('[CargoUI] Full rebuild complete');
    }
