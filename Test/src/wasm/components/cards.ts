...
export class Cards {
        ...
        for (let r = 0; r < this.acft_data.armour.length; ++r) {
            ...
            if (this.acft_data.armour[r] > 0) {
                if (str != "")
                    ...
                    str += localization.translate("Armour") + " ";
    ...
    public SaveWeapon(weapon_num: number) {
        ...
        context.fillText(damage[2], 157 + 160, 208 + 23, 80);
        context.fillText(damage[3], 157 + 240, 208 + 23, 80);
    ...
    public SaveEngine(engine_num: number, text: ENGINE_TEXT = ENGINE_TEXT.SINGLE) {
        ...
        if (this.eng_data.radiator >= 0) {
            context.fillText(localization.translateWithParam("Cards Uses Radiator", this.eng_data.radiator + 1), 109, 340, 270);
        }
