/**
 * Munitions UI Component
 *
 * Displays the Munitions section using UIBindings from Rust
 */
...
export class MunitionsUI {
    ...
    constructor(
        ...
        if (!bridge) {
            throw new Error('Bridge not available during MunitionsUI construction');
        }
    ...
    /**
     * Render the Munitions UI - intelligently updates or rebuilds
     */
    render(): void {
        const bridge = this.getBridge();
        if (!bridge || !bridge.isInitialized()) {
            console.warn('[MunitionsUI] Bridge not initialized, skipping render');
            return;
        }
    ...
    private rebuildFull(): void {
        ...
        const bridge = this.getBridge();
        if (!bridge || !bridge.isInitialized()) {
            console.warn('[MunitionsUI] Bridge not initialized, skipping rebuild');
            return;
        }

        const bindings = bridge.getMunitionsBindings();
        ...
        rulesLink.href = './Rules/Rules.htm#_Munitions';
        ...
        console.log('[MunitionsUI] Full rebuild complete');
    }
